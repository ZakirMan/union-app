...
content = re.sub(
    r"const \[activeTab, setActiveTab\] = useState<'news' \| 'chat' \| 'resources' \| 'training' \| 'profile' \| 'polls' \| 'reports'\>\('news'\);",
    r"const [activeTab, setActiveTab] = useState<'home' | 'resources' | 'profile' | 'polls' | 'reports'>('home');",
    content
)

content = re.sub(
    r"if \(tabParam && \['news', 'chat', 'resources', 'training', 'profile', 'polls', 'reports'\]\.includes\(tabParam\)\) \{",
    r"if (tabParam && ['home', 'resources', 'profile', 'polls', 'reports'].includes(tabParam)) {",
    content
)

# 3. Add states for admin request modal and month offset
state_insert_pos = content.find('const [showAidModal, setShowAidModal] = useState(false);')
if state_insert_pos != -1:
    new_states = """  const [showAdminRequestModal, setShowAdminRequestModal] = useState(false);
  const [monthOffset, setMonthOffset] = useState(0);
  
"""
    content = content[:state_insert_pos] + new_states + content[state_insert_pos:]
...
# 5. Build the new Home Tab and Admin Request Modal
news_start = content.find("{activeTab === 'news' && (")
news_end_match = re.search(r"^\s*\}\)\}\s*</div\>\s*</div\>\s*\)\}", content[news_start:], re.MULTILINE)
if news_end_match:
    news_end = news_start + news_end_match.end()

chat_start = content.find("{activeTab === 'chat' && (")
chat_end_match = re.search(r"^\s*</div\>\s*\)\}", content[chat_start:], re.MULTILINE)
if chat_end_match:
    chat_end = chat_start + chat_end_match.end()

training_start = content.find("{activeTab === 'training' && (")
training_end_match = re.search(r"^\s*</div\>\s*\)\}", content[training_start:], re.MULTILINE)
if training_end_match:
    training_end = training_start + training_end_match.end()
                ...
                <div className="space-y-4">
                  {{news.map((item) => (
                    <div key={{item.id}} className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100">
                      ...
                      <p className="text-gray-600 text-sm mb-4 line-clamp-3 whitespace-pre-wrap">{{item.body}}</p>
...
if news_start != -1 and chat_start != -1 and training_start != -1:
    last_div_idx = content.rfind('</div >')
    if last_div_idx == -1:
        last_div_idx = content.rfind('</div>')
    
    content = content[:last_div_idx] + admin_modal_code + content[last_div_idx:]

    content = content[:training_start] + content[training_end:]
    
    chat_start = content.find("{activeTab === 'chat' && (")
    chat_end_match = re.search(r"^\s*</div\>\s*\)\}", content[chat_start:], re.MULTILINE)
    chat_end = chat_start + chat_end_match.end()
    
    content = content[:chat_start] + content[chat_end:]
    
    news_start = content.find("{activeTab === 'news' && (")
    news_end_match = re.search(r"^\s*\}\)\}\s*</div\>\s*</div\>\s*\)\}", content[news_start:], re.MULTILINE)
    news_end = news_start + news_end_match.end()
    
    content = content[:news_start] + home_tab_code + content[news_end:]
