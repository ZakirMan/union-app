    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        const [lSnap, tSnap, nSnap, uSnap, cSnap] = await Promise.all([
          ...
          // Новости сортируем в JS, чтобы избежать ошибок индексов
        ...
        // 3. ОБРАЩЕНИЯ (ИСПРАВЛЕНО: УБРАН orderBy из запроса, добавлена сортировка JS)
        // Это гарантирует, что список загрузится даже без сложного индекса
    ...
    <div className="min-h-screen bg-[#F2F6FF] font-sans text-[#1A1A1A] pb-28">
               ...
               <div className="relative z-10">
                  ...
                  {!isEditing ? (
                    <>
                      ...
                      <button onClick={()=>setIsEditing(true)} className="bg-gray-100 text-gray-700 px-6 py-3 rounded-xl text-sm font-black hover:bg-gray-200 transition-colors">Редактировать профиль</button>
                    </>
                  ) : (
                    <div className="space-y-4 text-left max-w-sm mx-auto">
                      <div><label className="text-xs font-bold text-gray-400 uppercase ml-2">ФИО</label><input className="w-full p-4 border-0 bg-gray-50 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500" value={editName} onChange={e=>setEditName(e.target.value)} placeholder="Ваше Имя" /></div>
                      <div><label className="text-xs font-bold text-gray-400 uppercase ml-2">Должность</label><input className="w-full p-4 border-0 bg-gray-50 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500" value={editPosition} onChange={e=>setEditPosition(e.target.value)} placeholder="Ваша должность" /></div>
                      <div><label className="text-xs font-bold text-gray-400 uppercase ml-2">Телефон</label><input className="w-full p-4 border-0 bg-gray-50 rounded-2xl font-bold focus:ring-2 focus:ring-blue-500" value={editPhone} onChange={e=>setEditPhone(e.target.value)} placeholder="+7 (___) ___-__-__" /></div>
                      <div className="flex gap-3 pt-2"><button onClick={()=>{setIsEditing(false); setEditFile(null);}} className="flex-1 bg-gray-200 text-gray-700 py-4 rounded-2xl font-black hover:bg-gray-300 transition-colors">Отмена</button><button onClick={handleSaveProfile} disabled={isSavingProfile} className="flex-1 bg-blue-600 text-white py-4 rounded-2xl font-black hover:bg-blue-700 transition-colors shadow-lg shadow-blue-200 disabled:opacity-70">{isSavingProfile ? '...' : 'Сохранить'}</button></div>
                    </div>
                  )}
               </div>
            </div>
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-sm p-8 shadow-2xl relative animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmitDelegation} className="space-y-5">
              <div className="relative">
                ...
                <input type="text" placeholder="Начните вводить имя..." className="w-full p-4 border-0 rounded-2xl font-bold bg-gray-100 outline-none focus:ring-2 focus:ring-indigo-500 transition-all pr-10" value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setIsDropdownOpen(true); setSelectedDelegateId(''); }} onFocus={() => setIsDropdownOpen(true)} />
                {isDropdownOpen && searchTerm && (
                  <div className="absolute z-20 w-full bg-white border border-gray-100 rounded-2xl mt-2 max-h-60 overflow-y-auto shadow-xl ring-1 ring-black/5 py-2">
                    {filteredColleagues.length > 0 ? filteredColleagues.map(c => (
                        <div key={c.id} className="px-4 py-3 hover:bg-indigo-50 cursor-pointer transition-colors flex flex-col" onClick={() => { setSelectedDelegateId(c.id); setSearchTerm(c.displayName); setIsDropdownOpen(false); }}>
                          <span className="font-black text-gray-900">{c.displayName}</span><span className="text-xs font-bold text-indigo-500">{c.position}</span>
                        </div>
              ...
              <div>
                ...
                <label className="flex items-center justify-center w-full p-4 bg-gray-100 border-2 border-dashed border-gray-300 rounded-2xl cursor-pointer hover:border-indigo-400 hover:bg-indigo-50 transition-all group">
                    <span className="text-sm font-bold text-gray-500 group-hover:text-indigo-600 transition-colors">{delegateFile ? `Выбран файл: ${delegateFile.name}` : '📎 Нажмите, чтобы выбрать файл'}</span>
                    <input type="file" onChange={e => setDelegateFile(e.target.files?.[0] || null)} className="hidden"/>
                </label>
              </div>
              <div className="flex gap-3 pt-4"><button type="button" onClick={() => setShowDelegateModal(false)} className="flex-1 py-4 bg-gray-100 rounded-2xl font-black text-gray-700 hover:bg-gray-200 transition-colors">Отмена</button><button disabled={isSubmittingDelegation || !selectedDelegateId} className="flex-1 py-4 bg-gradient-to-r from-indigo-600 to-blue-600 text-white rounded-2xl font-black hover:from-indigo-700 hover:to-blue-700 transition-all shadow-lg shadow-indigo-200 disabled:opacity-50 disabled:shadow-none">{isSubmittingDelegation ? 'Отправка...' : 'Отправить'}</button></div>
            </form>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-md border-t border-white/20 p-2 flex justify-around items-center pb-safe z-40 shadow-lg shadow-black/5 rounded-t-[2rem] mx-2 mb-2">
        {[ { id: 'news', icon: '📰', label: 'Новости' }, { id: 'chat', icon: '💬', label: 'Чат' }, { id: 'resources', icon: '📂', label: 'Ресурсы' }, { id: 'profile', icon: '👤', label: 'Профиль' } ].map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex flex-col items-center justify-center w-16 h-16 rounded-2xl transition-all duration-300 group ${activeTab === tab.id ? 'bg-blue-50 text-blue-600 scale-110 shadow-sm' : 'text-gray-400 hover:text-gray-600 hover:bg-gray-50'} active:scale-95`}>
            <span className={`text-2xl mb-0.5 transition-transform duration-300 ${activeTab === tab.id ? 'scale-110 -rotate-6' : 'group-hover:scale-105'}`}>{tab.icon}</span><span className={`text-[10px] font-black tracking-wide transition-opacity ${activeTab === tab.id ? 'opacity-100' : 'opacity-70'}`}>{tab.label}</span>
          </button>
        ))}
      </div>
    </div>
